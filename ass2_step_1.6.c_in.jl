...
for i = 1:iteration
    ...
    for a = 1:A
        ...
        #----------------------------- Results -----------------------------#
            ...
            # CVaR
            CVaR_1_6c_in[a, b] = (value.(zeta) - 1/(1-alpha[a]) * sum(prob*value.(eta[s]) for s = 1:S))

            # expected profit
            exp_profit_1_6c_in[a, b] = sum(sum(prob *
...
println("Expected profit: ", exp_profit_1_6c_in)
println(size(exp_profit_1_6c_in))
println("Mean expected profit ",mean(exp_profit_1_6c_in))