...
@constraint(Step_2_2, [s=1:S], alpha_s_offer[s] >= 0)

@constraint(Step_2_2, [d=1:D], - demand_bid[d] + mu_d_cap[d] - mu_d_undercap[d] 
            + lambda[node_demands(d)] == 0)  # Dual of demand capacity constraint

@constraint(Step_2_2, [s=1:S], alpha_s_offer[s] + mu_s_cap[s] - mu_s_undercap[s] 
            - lambda[node_strat_gen(s)] == 0) # Dual of strategic capacity constraint

@constraint(Step_2_2, [o=1:O], non_strat_gen_cost[o] + mu_o_cap[o] - mu_o_undercap[o] 
            - lambda[node_non_strat_gen(o)] == 0) # Dual of non-strategic capacity constraint

@constraint(Step_2_2, [n=1:N],
...
@constraint(Step_2_2, [d=1:D], 0 <= demand_cons[d] - pd[d]) # Demand capacity constraint
...
if termination_status(Step_2_2) == MOI.OPTIMAL
    ...
    println("Objective value: ", objective_value(Step_2_2))
